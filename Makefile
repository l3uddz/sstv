...
${BUILD_PATH}/${CMD}: ${GO_FILES} go.sum
	...
	CGO_ENABLED=${CGO} go build \
		-mod vendor \
		-trimpath \
		-ldflags "-s -w -X github.com/l3uddz/sstv/build.Version=${VERSION} -X github.com/l3uddz/sstv/build.GitCommit=${GIT_COMMIT} -X github.com/l3uddz/sstv/build.Timestamp=${TIMESTAMP}" \
		-o ${BUILD_PATH}/${CMD} \
		./cmd/sstv
